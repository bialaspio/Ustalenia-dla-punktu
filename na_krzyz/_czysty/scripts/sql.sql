...
drop table if exists lnie_garanic_ustalenia;
drop table if exists punkty;
drop table if exists punkty_graniczne_pot;

drop table if exists dzialki_aaa;
create table dzialki_aaa as 
...
update dzialki_aaa A set nr_stanowiska = (select string_agg(distinct text, ' - ') from stanowiska B where ST_Within( B.wkb_geometry, A.wkb_geometry));
-- kontrola czy sa jeszcze podwujne jak tak poprawka 
select *from dzialki_aaa where nr_stanowiska like '%-%';

-- poprawa blednych 
update dzialki_aaa A set nr_stanowiska = '02B15' where nr_stanowiska = '02B15 - 02C16';


-- stworzenie tabli z liniami dla danego obrebu 
drop table if exists lnie_garanic_ustalenia_obr;
create table lnie_garanic_ustalenia_obr as select distinct A.* from lnie_garanic_ustalenia A, dzialki_aaa B where ST_Within(A.wkb_geometry , st_buffer (B.wkb_geometry,0.02)) and length(nr_dzialki) < 12;
...
alter table dzialki_aaa add ogc_fid serial;
drop table if exists dzialki_poligony_buf01;
create table dzialki_poligony_buf01 as 
select ogc_fid, st_buffer (wkb_geometry, '0.1') as wkb_geometry from dzialki_aaa where length(nr_dzialki) < 12  ;
...
select DISTINCT A.* from dzialki_aaa A, dzialki_poligony_buf01 B where st_intersects(A.wkb_geometry, B.wkb_geometry) and A.ogc_fid not in (
...
select wkb_geometry, nr_dzialki, nr_stanowiska from dzialki_aaa where ogc_fid in (select ogc_fid from dzialki_poligony_buf01);
...
select *from aaa_dz_bez_duble A where dz_1||dz_2 not in (select dzialka_a||dzialka_b from dziki_pary_por) AND dz_2||dz_1 not in (select dzialka_a||dzialka_b from dziki_pary_por) --72

create table skr_aaa as 
select dz_1, dz_2, wartosc from aaa_dz_bez_duble A where dz_1||dz_2 in (select dzialka_a||dzialka_b from dziki_pary_por) OR dz_2||dz_1 in (select dzialka_a||dzialka_b from dziki_pary_por) --4477 


alter table skr_aaa add s varchar;
update skr_aaa A set s = (SELECT B.s from skr B where B.d1||B.d2 like A.dz_1||A.dz_2 or B.d2||B.d1 like A.dz_1||A.dz_2)
update skr_aaa A set s = (SELECT distinct B.stanowisko from rw_b_wskr B where B.dzialka_a||B.dzialka_b like A.dz_1||A.dz_2 or B.dzialka_b||B.dzialka_a like A.dz_1||A.dz_2) where s is null 

select dz_1, dz_2, s from skr_aaa where s is null 
select *from rw_b_wskr 
select *from skr 

...
def crate_dzialki_pary():
	...
	update_query = "update dzialki_pary A set G_dz_a =(SELECT DISTINCT grupa from dzialki_aaa B where A.dzialka_a like B.nr_dzialki)"
	exec_query_commit(update_query)
	update_query = "update dzialki_pary A set G_dz_b =(SELECT DISTINCT grupa from dzialki_aaa B where A.dzialka_b like B.nr_dzialki)"
	exec_query_commit(update_query)
...
def generuj_pliki_csv():
	...
	select_query = "select distinct *from dzialki_pary where id in (select distinct B.id from skr A, dzialki_pary B where (A.D1=B.dzialka_a and A.D2=B.dzialka_B) OR (A.D2=B.dzialka_a and A.D1=B.dzialka_B))"
	cur_local_select.execute(select_query) 
	...
	select_query = "select distinct dzialka_a, dzialka_b, g_dz_a, g_dz_b from dzialki_pary where id in (select distinct B.id from skr A, dzialki_pary B where (A.D1=B.dzialka_a and A.D2=B.dzialka_B) OR (A.D2=B.dzialka_a and A.D1=B.dzialka_B))"

...
def get_data_from_protokoly():
	dirs_name = glob.glob ('xls_protokoly')
	print (str (dirs_name))
	#Stworzenie tablicy na dane z katalogu ZAWIADOMIENIA 
	drop_query = 'DROP TABLE IF EXISTS PROTOKOLY'
	exec_query_commit(drop_query)
	create_query = 'create table PROTOKOLY (NR_dzialki varchar, podmioty varchar, Nr_KW varchar, DataUstalenia varchar, Nr_szkicu varchar, Adnotacje varchar)'
		...
		for file_xls_name in files_xls_names:
			...
			for index, row in xls_zaw.iterrows():
				if index >= 6:
					...
					insert_query = "INSERT INTO public.protokoly(nr_dzialki, podmioty, nr_kw, dataustalenia, nr_szkicu, adnotacje) VALUES ('"+NR_dzialki+"','"+podmioty+"','"+Nr_KW+"','"+DataUstalenia+"','"+Nr_szkicu+"','"+Adnotacje+"');"
					print ('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
					print (insert_query)
					print ('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
